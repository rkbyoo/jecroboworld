      ...
      <div className="container mx-auto px-4">
          ...
          <nav className="hidden md:flex items-center space-x-1">
            {navItems.map((item) => (
              <Link
                key={item.name}
                to={item.path}
                className={`block px-3 py-2 text-base font-medium transition-all duration-300 relative will-change-transform backface-visibility-hidden transform-gpu ${
                  isActive(item.path)
        ...
        {isMenuOpen && (
          <div className="md:hidden">
            <div className="px-2 pt-2 pb-3 bg-secondary border-t border-primary/20">
              {navItems.map((item) => (
                <div key={item.name} className="block">
                  <Link
                    to={item.path}
                    className={`inline-block px-3 py-2 text-base font-medium transition-all duration-300 relative will-change-transform backface-visibility-hidden transform-gpu ${
                      isActive(item.path)
                        ? "text-secondary-foreground scale-105 after:absolute after:bottom-0 after:left-0 after:h-0.5 after:bg-primary after:rounded-full after:w-full"
                        : "text-secondary-foreground/80 hover:text-secondary-foreground hover:scale-105"
                    }`}
